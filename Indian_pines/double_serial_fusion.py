...
#kpca=KernelPCA(n_components=90, kernel='linear')
...
q = pd.concat([pd.DataFrame(data = lleComponents_s), pd.DataFrame(data = y.ravel())], axis = 1)
q.columns = [f'S-{i}' for i in range(1,110+1)]+['class']
#print(q.head())

#Change ' data=' and 'range'
gab = pd.concat([pd.DataFrame(data = lleComponents_g), pd.DataFrame(data = y.ravel())], axis = 1)
gab.columns = [f'G-{i}' for i in range(1,110+1)]+['class']
#print(gab.head())
...
#Apply feature extraction to the serially fused spectral-textural features
#principalComponents_f = pca.fit_transform(df_fused.iloc[:, :-1].values)
#independentComponents_f = ica.fit_transform(df_fused.iloc[:, :-1].values)
#lleComponents_f = lle.fit_transform(df_fused.iloc[:, :-1].values)
kpcaComponents_f = kpca.fit_transform(df_fused.iloc[:, :-1].values)

#Change ' data=' and 'range'
fus = pd.concat([pd.DataFrame(data = lleComponents_f), pd.DataFrame(data = y.ravel())], axis = 1)
fus.columns = [f'F-{i}' for i in range(1,110+1)]+['class']
#print(fus.head())
